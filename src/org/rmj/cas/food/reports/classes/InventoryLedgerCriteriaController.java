 ...
 * @author User
 */
public class InventoryLedgerCriteriaController implements Initializable {
...
//                }
    ...
    private JSONObject searchStock(String fsValue, boolean fbByCode) {
        String lsSQL = " SELECT"
                + "  a.sStockIDx sStockIDx"
                + ", a.sBarCodex sBarCodex"
                + ", a.sDescript sDescript"
                + ", IFNULL(b.sDescript,'') xBrandNm"
                + ", IFNULL(c.sDescript,'') xModelNm"
                + ", IFNULL(d.sDescript,'') xColorNm"
                + ", IFNULL(e.sMeasurNm,'') sMeasurNm"
                + ", IFNULL(f.sDescript,'') xInvType"
                + " FROM Inventory a"
                + " LEFT JOIN Brand b"
                ...
                + " LEFT JOIN Color d"
                + " ON a.sColorCde = d.sColorCde"
                + " LEFT JOIN Measure e "
                + " ON a.sMeasurID = e.sMeasurID "
                + " LEFT JOIN Inventory_Type f"
                + " ON a.sInvTypCd = f.sInvTypCd";

        return showFXDialog.jsonSearch(oApp,
                lsSQL,
                fsValue,
                "Barcode»Description»Brand»Model»Measure»Color»Inv. Type",
                "sBarCodex»sDescript»xBrandNm»xModelNm»sMeasurNm»xColorNm»xInvType",
                "a.sBarCodex»a.sDescript»b.sDescript»c.sDescript»e.sMeasurNm»d.sDescript»f.sDescript",
                fbByCode ? 0 : 1);
    }
    ...
    private void txtField_KeyPressed(KeyEvent event) {
        ...
        if (lnIndex == 3) {
            ...
            if (event.getCode() == KeyCode.F3) {
                loJSON = searchStock(lsValue, true);

                if (loJSON != null) {
                    psStockID = (String) loJSON.get("sStockIDx");
                    txtField04.setText((String) loJSON.get("sBarCodex"));
                    txtField05.setText((String) loJSON.get("sDescript"));
                } else {
                    psStockID = "";
                    txtField04.setText("");
                    txtField05.setText("");
            ...
            if (event.getCode() == KeyCode.F3) {
                loJSON = searchStock(lsValue, false);

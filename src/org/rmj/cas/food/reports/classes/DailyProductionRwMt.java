...
public class DailyProductionRwMt implements GReport {
    ...
    private LogWrapper logwrapr = new LogWrapper("org.rmj.foodreports.classes.DailyProductionRwMt", "DailyProductionRwMtReport.log");
    ...
    public boolean getParam() {
        if (_instance.getUserLevel() < 2) {
            _message = "Invalid user was detected... \n"
                    + "Only Supervisor can generate this Report";

            ShowMessageFX.Information(_message, "Information", "");
            return false;

        }
        FXMLLoader fxmlLoader = new FXMLLoader(getClass().getResource("DailyProductionRwMtCriteria.fxml"));
        fxmlLoader.setLocation(getClass().getResource("DailyProductionRwMtCriteria.fxml"));

        DailyProductionRwMtCriteriaController instance = new DailyProductionRwMtCriteriaController();
        ...
        try {
            ...
            ShowMessageFX.Error(e.getMessage(), DailyProductionRwMt.class.getSimpleName(), "Please inform MIS Department.");
            System.exit(1);
        }
        ...
        Task<Boolean> reportTask = new Task<Boolean>() {
            ...
            protected Boolean call() throws Exception {
                ...
                if (_instance.getUserLevel() < 2) {
                    ...
                    _message = "Invalid user was detected... /n"
                            + "Only Supervisor can generate this Report";
                    return false;
...
//            System.err.println("Failed to load the report: " + reportTask.getException().getMessage());
    ...
    private boolean printDetail() throws SQLException {
        ...
        if (!System.getProperty("store.report.criteria.datefrom").equals("")
            ...
            lsCondition = "b.dTransact BETWEEN " + lsCondition;
        } else {
            lsCondition = "0 = 1";
        }
        String lsSQL = getReportSQLDetailed();
        lsSQL = MiscUtil.addCondition(lsSQL, lsCondition);
        lsSQL = MiscUtil.addCondition(lsSQL, " b.cTranStat = '1'");
        ...
        excelName = "Daily Production Detailed RwMt - " + lsExcelDate + ".xlsx";
        if (System.getProperty("store.report.criteria.isexport").equals("true")) {
            String[] headers = {"Branch", "Date", "Transaction No", "Barcode", "Description", "Brand", "Measure", "Qty Req.", "Qty Used", "Transacted by."};
        ...
        try {
            ...
            Logger.getLogger(DailyProductionRwMt.class.getName()).log(Level.SEVERE, null, ex);

            Platform.runLater(() -> {
                ShowMessageFX.Error(ex.getMessage(), DailyProductionRwMt.class.getSimpleName(), "Please inform MIS Department.");
            });
            return false;
    ...
    private String getReportSQLDetailed() {
        String lsSQL = "SELECT"
                ...
                + " , DATE_FORMAT(b.dTransact, '%Y-%m-%d') sField03"
                + " , IFNULL(c.sBarCodex,'') sField04"
                + " , IFNULL(c.sDescript,'') sField05"
                + " , IFNULL(e.sDescript, '') sField06"
                + " , IFNULL(d.sMeasurNm, '') sField07"
                + " , IFNULL(i.sCompnyNm, '') sField08"
                + " , SUM(IFNULL(a.nQtyReqrd,0)) nField01"
                + " , SUM(IFNULL(a.nQtyUsedx,0)) nField02"
                + "  FROM Daily_Production_Inventory a"
                + "  LEFT JOIN Daily_Production_Master b ON a.sTransNox = b.sTransNox"
                ...
                + "  LEFT JOIN xxxSysUser g ON  b.sModified = g.sUserIDxx"
                + "  LEFT JOIN GGC_ISysDBF.Employee_Master001 h ON h.sEmployID = g.sEmployNo"
                + "  LEFT JOIN GGC_ISysDBF.Client_Master i ON  h.sEmployID = i.sClientID"
...
//        System.out.println("getHeightInPoints = " + sheet.getRow(0).getHeightInPoints());
        ...
        for (DailyProductionModel item : data) {
            ...
            if (System.getProperty("store.report.criteria.presentation").equals("1")) {
                ...
                row.createCell(9).setCellValue(item.getsField08().toString());
